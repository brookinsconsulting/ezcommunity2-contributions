...
if ( count( $latest ) > 0 )
{
  ...
  foreach ( $latest as $visitor )
  {
	...
	$pageview_user = $visitor["UserID"];
	$pageview_user = new eZUser($pageview_user);

	$pageview_user_name = $pageview_user->name();
	$pageview_user_login = $pageview_user_name;
	$pageview_user_login = $visitor["UserID"];

//if ( $pageview_user_name == ' '){
	$pageview_user_login = 'cravass, huge gapeing hole';
//}
...
//	$pageview_user_login = $pageview_user->login();
    ...
    $t->set_var( "remote_user", $pageview_user_login );
