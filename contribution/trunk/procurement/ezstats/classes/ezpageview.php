...
class eZPageView
{
    ...
    */
    function store()
    {
	    ...
	    // Download File name Encodeing Match Detection & Patch


            function getRfpStatInfo2001($uri) {
                $uri_arr = explode("/", $uri);
                $info = array();
		...
		$info['file_encoded'] = rawurlencode( $uri_arr[4] );
                $info['file_decoded'] = rawurldecode( $uri_arr[4] );

                return $info;
            }


            $requestUriExploded = getRfpStatInfo2001($requestURI);
	    $url_obscured_match = false;

	    if ( $requestUriExploded['module'] == 'filemanager' ) {

	    $requestUriExplodedFile = $requestUriExploded['file'];
	    $requestUriEncoded = rawurlencode( $requestUriExplodedFile );
	    $requestUriDecoded = $requestUriExplodedFile;
	    $requestUriDecodedR = rawurldecode( $requestUriExplodedFile );
	    
		// urldecode
	    $requestDownloadLinkReBuild = '/' . $requestUriExploded['module'] . '/download/'. $requestUriExploded['id'] .'/'. $requestUriEncoded; 

  $requestDownloadLinkDecodeReBuild = '/' . $requestUriExploded['module'] . '/download/'. $requestUriExploded['id'] .'/'. $requestUriDecodedR;

/*
            v_arrayz($requestUriExploded); // exit();
...
// if ( ) { }
            print('$requestUriExplodedFile : '. $requestUriExplodedFile);
                print("<br />");

	    print('$requestUriEncoded : '. $requestUriEncoded); 
                print("<br />");

 	    print('requestDownloadLinkReBuild : '. $requestDownloadLinkReBuild); 

                 print("<br />");
	    print('requestUriDecodedR : ' . $requestUriDecodedR);

		 print("<br />");
		 print("<br />");


                $db->array_query( $request_page_array2,
		"SELECT ID FROM eZStats_RequestPage
		WHERE URI='$requestDownloadLinkReBuild'" );

		$count_encoded = sizeof($request_page_array2);
		$count_request = sizeof($request_page_array);

		if ($requestURI == $requestDownloadLinkDecodeReBuild ) { 

		if ( $count_request == $count_encoded ) {
			$url_obscured_match = true;
		}

  		 if ( sizeof($request_page_array) > 0 ) {
		    print('$count_encoded = ' . $count_encoded); 	
  		     v_arrayz(  $request_page_array );
		     v_arrayz(  $request_page_array[0] ); 
		      print("<br />");
		    print('$count_request = ' . $count_request);
                     v_arrayz(  $request_page_array2 );
                     v_arrayz(  $request_page_array2[0] );
		 }
		}
//		exit();
		print( $requestURI ); 
*/
            ...
            if ( ( count( $request_page_array ) == 0 ) && ( $url_obscured_match == false ) )
            {
