...
class eZPageViewQuery
{
    ...
    */
    function &topVisitors( $limit = 20, $offset = 0 )
    {
	    ...
	    $user = $visitor_array[$i][$db->fieldName( "UserID" )];
    ...
    */
    function &topProductRequests( $limit=20 )
    {
	    ...
	    /*!
      Returns the most frequent viewed rfps.
    */
    function &topRfpRequests( $limit=100 )
    {
        $db =& eZDB::globalDatabase();
...
// "SELECT count(eZStats_PageView.ID) AS Count, eZStats_PageView.UserID, eZStats_RequestPage.ID, eZStats_RequestPage.URI  
// eZStats_RequestPage.URI LIKE '/rfp/view/%
//OR eZStats_RequestPage.URI LIKE '/rfp/download/%'

// OR eZStats_RequestPage.URI LIKE '/rfp/view/%'
...
//GROUP BY eZStats_RequestPage.ID
//GROUP BY eZStats_RequestPage.URI
...
//                                       "Count" => $visitor_array[$i][$db->fieldName( "Count" )]
      ...
      Returns the most frequent viewed rfps.
    */
    function &topRfpDownloads( $rfpID, $limit=20 )
    {
        $db =& eZDB::globalDatabase();
...
//		$rfpID = $rfpID;
        ...
        "SELECT count(eZStats_PageView.ID) AS Count, eZStats_PageView.UserID, eZStats_RequestPage.ID, eZStats_RequestPage.URI
         FROM eZStats_PageView, eZStats_RequestPage
         WHERE eZStats_PageView.RequestPageID=eZStats_RequestPage.ID
         AND eZStats_RequestPage.URI LIKE '/rfp/rfpview/$rfpID'
		 OR eZStats_RequestPage.URI LIKE '/rfp/download/$rfpID'
         GROUP BY eZStats_RequestPage.ID
         ORDER BY Count DESC",
        array( "Limit" => $limit,
               "Offset" => $offset ) );
