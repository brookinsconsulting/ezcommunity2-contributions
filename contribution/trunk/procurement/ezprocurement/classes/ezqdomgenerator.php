...
//
//!! eZRfp
//! eZTechGenerator generates  XML contents for rfps.
/*!
  This class will generate a tech XML rfp. This class is ment
...
class eZQDomGenerator
{
    ...
    */
    function eZQDomGenerator( &$contents )
    {
        ...
        $customTags = $ini->read_var( "eZRfpMain", "CustomTags" );
    ...
    */
    function &generateXML()
    {
        ...
        $newContents .= "<rfp><generator>qdom</generator>\n";
        ...
        $newContents .= "<body>" . $body . "</body></rfp>";
...
//        $tmpPage = preg_replace( "#/&gt;#", "/>", $tmpPage );
    ...
    */
    function &generateLink( $tmpPage )
    {
        ...
        // convert <ezrfp id text> to valid xml
        $tmpPage = preg_replace( "#(<ezrfp\s+?([^ ]+)\s+?([^>]+)>)#", "<ezrfp id=\"\\2\" text=\"\\3\" />", $tmpPage );
    ...
    */
    ...
    /*!
      Decodes the xml chunk and returns the original array to the rfp.
...
//        $xml =& qdom_tree( $this->Contents );
        ...
        else
        {
            ...
            foreach ( $xml->children as $child )
            {
                if ( $child->name == "rfp" )
                {
                    foreach ( $child->children as $rfp )
                    {
                        if ( $rfp->name == "intro" )
                        {
                            $intro = $this->decodePage( $rfp );
                        }
    ...
    */
    function &decodeLink( $paragraph )
    {
        ...
        // ezrfp
        if ( $paragraph->name == "ezrfp" )
        {
            foreach ( $paragraph->attributes as $ezRfpItem )
            {
                switch ( $ezRfpItem->name )
                {

                    case "id" :
                    {
                        $id = $ezRfpItem->children[0]->content;
                    }
                    break;

                    case "text" :
                    {
                        $text = $ezRfpItem->children[0]->content;
                    }
                    break;
                }
            }

            $pageContent .= "<ezrfp $id $text>";
        }
    ...
    */
    ...
    /*!
      Returns the number of pages found in the rfp.
    */
    function pageCount( )
    {
