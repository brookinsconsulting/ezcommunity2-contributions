...
{
    if ( ( $ParentID == 0 ) && ( eZPermission::checkPermission( $user, "eZRfp", "WriteToRoot" ) == false ) )
    {
...
if ( $Action == "insert" && !$error )
{
    ...
    $parentCategory = new eZRfpCategory();
    ...
    if ( isSet( $WriteGroupArray ) )
    {
        ...
        {
            eZObjectPermission::setPermission( -1, $categoryID, "rfp_category", 'w' );
        }
        else
        {
            eZObjectPermission::removePermissions( $categoryID, "rfp_category", 'w' ); 
            foreach ( $WriteGroupArray as $groupID )
            {
                eZObjectPermission::setPermission( $groupID, $categoryID, "rfp_category", 'w' );
            }
        }
    }
    else
    ...
    if ( isSet( $GroupArray ) )
    {
        if ( $GroupArray[0] == 0 )
        {
            eZObjectPermission::setPermission( -1, $categoryID, "rfp_category", 'r' );
        }
        else // some groups are selected.
        {
            eZObjectPermission::removePermissions( $categoryID, "rfp_category", 'r' ); 
            foreach ( $GroupArray as $groupID )
            {
                eZObjectPermission::setPermission( $groupID, $categoryID, "rfp_category", 'r' );
            }
        }
    }
    else
    ...
    if ( isSet( $Browse ) )
    {
        ...
        $session->setVariable( "ImageListReturnTo", "/rfp/categoryedit/edit/$categoryID/" );
...
//        $category->setOwnerGroup( $ownerGroup, false );
    ...
    $categoryID = $category->id();
    /* write access select */
    eZObjectPermission::removePermissions( $categoryID, "rfp_category", 'w' ); //not really necessary..
    if ( isset( $WriteGroupArray ) )
    {
        if ( $WriteGroupArray[0] == 0 )
        {
    ...
    $categoryID = $category->id();
    $files =& eZCacheFile::files( "ezrfp/cache/",
                                  array( "rfplist", array( $CategoryID, $ParentID ), NULL ),
                                  "cache", "," );
    foreach ( $files as $file )
    {
        $file->delete();
    ...
    eZHTTPTool::header( "Location: /rfp/archive/$ParentID/" );
    exit();
}
...
$t = new eZTemplate( "ezrfp/admin/" . $ini->read_var( "eZRfpMain", "AdminTemplateDir" ),
                     "ezrfp/admin/intl/", $Language, "categoryedit.php" );
...
if ( $Action == "edit" )
{
    ...
    $writeGroupsID = eZObjectPermission::getGroups( $CategoryID, "rfp_category", 'w' , false );
    $readGroupsID = eZObjectPermission::getGroups( $CategoryID, "rfp_category", 'r', false );
...
{
    if ( eZObjectPermission::hasPermission( $item[0]->id(), "rfp_category", 'w' ) && $CategoryID != $item[0]->id() )
    {
