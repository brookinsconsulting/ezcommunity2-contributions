...
class eZRfp
{
    ...
    */
	...
	die("this is the v_array: ".v_array($contentsWriters));
...
//            		$writers = $this->ContentsWriters;
    ...
    */
    function &search( &$queryText, $sortMode=time, $fetchPublished=false, $offset=0, $limit=10, $params = array(), &$SearchTotalCount )
    {
        ...
        // Build the ORDER BY
        $OrderBy = "eZRfp_RfpWordLink.Frequency DESC";
        switch( $sortMode )
        {
            case "alpha" :
            {
        ...
        if ( isSet( $params["Categories"] ) )
        {
            ...
            if ( count( $cats ) > 0 )
            {
                $catSQL = "AND ( $sql ) AND Category.ID=eZRfp_RfpCategoryLink.CategoryID
