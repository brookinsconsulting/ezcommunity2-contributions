                ...
                if ( !$user->exists( $user->login() ) )
                {
                    ...
                    if ( eZMail::validate( $tmp[0] ) )
                    {
                        ...
                        if ( $InfoDisclaimer == "on" )
                            $user->setInfoDisclaimer( true );
                        else
                            $user->setInfoDisclaimer( false );
                        ...
                        if ( $DeadlineReminders == "on" )
                            $user->setDeadlineReminders( true );
                        else
                            $user->setDeadlineReminders( false );
...
//                            $user->removeGroups();
                        ...
                        eZHTTPTool::header( "Location: /user/userlist/0/name" );
                        exit();
                    }
                    else
                    {
...
if ( $Action == "delete" )
{
    if ( eZPermission::checkPermission( $user, "eZUser", "UserDelete" ) )
    {
        ...
        eZLog::writeNotice( "User deleted: $firstname $lastname ($login) $CompanyName $email $simultaneousLogins from IP: $REMOTE_ADDR" );
        eZHTTPTool::header( "Location: /user/userlist/" );
        exit();
    }
    else
...
$group = new eZUserGroup();
// don't get all groups, just the one group i dneed , hard code.!
...
if ( $Action == "edit" )
{
    ...
    if( $user->infoDisclaimer() == true )
        $InfoDisclaimer = "checked";
    else
        $InfoDisclaimer = "";

    if( $user->deadlineReminders() == true )
...
$t->set_var( "info_deadlinereminders", $DeadlineReminders );
$t->set_var( "account_active", $AccountActive );
