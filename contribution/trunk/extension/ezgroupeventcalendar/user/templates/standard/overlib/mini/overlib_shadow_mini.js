...
if (olIe4&&!isMac) {dpObj.style.filter='alpha(Opacity='+op+')';dpObj.filters.alpha.enabled=true;} else {if (typeof(dpObj.style.MozOpacity)=='string') dpObj.style.MozOpacity=op/100;}
}
}
o3_width=wd+X;o3_aboveheight=ht+Y;}
function cleanUpShadowEffects() {if (olNs4) return;else {var dpObj=(olIe4?o3_frame.document.all['backdrop']:o3_frame.document.getElementById('backdrop'));if (olIe4&&!isMac&&dpObj.filters.alpha) dpObj.filters.alpha.enabled=false;else if (typeof(dpObj.style.MozOpacity)=='string') dpObj.style.MozOpacity=1.0;}
}
