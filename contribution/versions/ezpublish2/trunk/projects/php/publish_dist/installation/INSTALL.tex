...
\title{eZ publish 2.1 Installation Guide}
...
\item libXml2 (http://xmlsoft.org/\#Downloads) version 2.2.7, 2.2.8 or 2.2.9.
...
source code version from this site, for windows you can just download
the binary. (eZ publish uses references for objects and foreach loops.
...
by directory basis in either the {}``.htaccess{}'' files (if you
use them) or in the setup of the virtual server in {}``httpd.conf{}''.
...
The directory {}``/your/apache/documentroot/publish\_dist{}'' is
the directory where you extracted eZ publish.
...
inclusion. The only correct way to access the admin site is through
its own virtual host address.}
...
\texttt{\footnotesize ~~~~RewriteEngine On} {\selectlanguage{english}}

\texttt{\footnotesize ~~~~RewriteRule \textasciicircum{}/stats/store/(.{*}).gif\$} {\selectlanguage{english}}

\texttt{\footnotesize ~~~~/publish/ezstats/user/storestats.php
{[}S=2{]}} {\selectlanguage{english}}

\texttt{\footnotesize ~~~~\# The two lines above should appear
on the} {\selectlanguage{english}}
...
Then we need to run {}``secure\_modfix.sh{}'' to make sure that
...
in the directory {}``updates{}''. Run the files needed to update
your version to the current. You need to apply all the updates for
every version since your version.
...
\texttt{\footnotesize ~~~~\#attempt to modify} {\selectlanguage{english}\texttt{\footnotesize }}{\footnotesize \par}
...
Thanks to Øyvind Wærendskjold \texttt{\footnotesize <werenskjold@disippel.net>}
...
You should also check the virtual host configuration as described
in section \ref{sec: dual virtualhost}, since the virtual host configuration
is explained thoroughly there and there might be some newer information
there.
