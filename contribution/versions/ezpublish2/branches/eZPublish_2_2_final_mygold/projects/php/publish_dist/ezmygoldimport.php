<?php
//
// $Id: ezmygoldimport.php,v 1.1.2.1 2001/11/27 15:33:42 ce Exp $
//
// Definition of ||| class
//
// <real-name> <<mail-name>>
...
{
    function eZMygoldImport( $host="www.mygold.com", $user=false, $password=false, $everything=false, $deleteOnly=false, $limit=0 )
    {
        ...
        $this->CanAdd = $everything;
        $this->DoNotAdd = $deleteOnly;
    ...
    function unavailable( $id )
    {
        if ( is_string ( $id ) )
        {
            ...
            $paramenter = new eZXMLRPCString( $id );
 ...
 */
        $this->db->array_query( $ret_array, "SELECT *, CONCAT( Wg, '-', Nummer, '-', Eigentuemer ) AS ProductNumber, CONCAT( Wg, '-', Nummer, '-', Eigentuemer, '-', Global ) AS RemoteID FROM Aktiv
...
*/
        $this->db->array_query( $ret_array, "SELECT *, CONCAT( Wg, '-', Nummer, '-', Eigentuemer ) AS ProductNumber, CONCAT( Wg, '-', Nummer, '-', Eigentuemer, '-', Global ) AS RemoteID FROM Aktiv WHERE
 ...
 */
        $this->db->array_query( $ret_array, "SELECT *, CONCAT( Wg, '-', Nummer, '-', Eigentuemer ) AS ProductNumber, CONCAT( Wg, '-', Nummer, '-', Eigentuemer, '-', Global ) AS RemoteID, SUBSTRING_INDEX( Referenz, '/', -1 ) AS SubString,  CONCAT( Farbe, TRIM( TRAILING SUBSTRING_INDEX( Referenz, '/', -1 )
 ...
 */
        $this->db->array_query( $ret_array, "SELECT *, CONCAT( Wg, '-', Nummer, '-', Eigentuemer ) AS ProductNumber,
                               CONCAT( Wg, '-', Nummer, '-', Eigentuemer, '-', Global ) AS RemoteID
                               FROM Aktiv
