...
$t = new eZTemplate( "eztrade/admin/" . $ini->read_var( "eZTradeMain", "AdminTemplateDir" ),
                     "eztrade/admin/intl/", $Language, "vouchercreatebyemail.php" );

$t->set_file( "vouchercreatebyemail_tpl", "vouchercreatebyemail.tpl" );

$t->setAllStrings();

$t->set_block( "vouchercreatebyemail_tpl", "error_tpl", "error" );
$t->set_block( "vouchercreatebyemail_tpl", "form_tpl", "form" );
$t->set_block( "vouchercreatebyemail_tpl", "address_success_tpl", "address_success" );
$t->set_block( "vouchercreatebyemail_tpl", "success_tpl", "success" );
...
$t->set_block( "error_tpl", "to_surname_empty_tpl", "to_surname_empty" );
$t->set_block( "error_tpl", "to_lastname_empty_tpl", "to_lastname_empty" );
...
$t->set_var( "to_surname_empty", "" );
$t->set_var( "to_lastname_empty", "" );
$t->set_var( "to_email_wrong", "" );
$t->set_var( "error", "" );
$t->set_var( "success", "" );
$t->set_var( "form", "" );
$t->set_var( "address_success", "" );
...
if ( $Action == "Create" )
{
    ...
    {
        $t->set_var( "number", $i + 1 );    
        if ( trim( $myaddresses[$i][0] ) == "" )
	    $t->parse( "to_surname_empty", "to_surname_empty_tpl", true );
	if ( trim( $myaddresses[$i][1] ) == "" )
	    $t->parse( "to_lastname_empty", "to_lastname_empty_tpl", true );
	if ( validate( trim( $myaddresses[$i][2] ) ) != 1 )
	    $t->parse( "to_email_wrong", "to_email_wrong_tpl", true );
    }
	...
	reset( $myaddresses );
	    ...
	    $voucher->setTotalValue( $Price );	    
            ...
            $online = new eZOnline();
            $online->setUrl( trim( $myaddresses[$i][2] ) );
            $online->store();
	    ...
	    $ToName = trim( str_replace( "\r\n", "", $myaddresses[$i][0] ) )." ".trim( $myaddresses[$i][1] );
...
elseif ( $keys_succes )
{
    ...
    for ( $i = 0; $i < count( $myaddresses ); $i++ )
    {
    	$t->set_var( "prename", trim( $myaddresses[$i][0] ) );
	$t->set_var( "lastname", trim( $myaddresses[$i][1] ) );
	$t->set_var( "email", trim( $myaddresses[$i][2] ) );
	$t->parse( "address_success", "address_success_tpl", true );
    }
...
$t->pparse( "output", "vouchercreatebyemail_tpl");
