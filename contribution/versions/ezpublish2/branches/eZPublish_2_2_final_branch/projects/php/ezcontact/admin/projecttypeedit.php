<?php
//
// $Id: projecttypeedit.php,v 1.5.2.1 2002/05/06 13:39:52 jhe Exp $
//
...
if ( $Action == "insert" or $Action == "update" )
{
    ...
    else
    {
        ...
        $item_type->setExpiryTime( $ExpiryTime * 86400 );
        $item_type->setWarningTime( $WarningTime * 86400 );
    }
    $item_type->store();
    include_once( "classes/ezhttptool.php" );
    eZHTTPTool::header( "Location: $page_path/list" );
...
$t->set_var( "expiry_value", $ExpiryValue );
$t->set_var( "warning_value", $WarningValue );
...
else
{
    if ( is_numeric( $item_type->id() ) )
    {
        ...
        if ( $item_type->expiryTime() == 0 )
        {
            $t->set_var( "expiry_value", "14" );
            $t->set_var( "warning_value", "5" );
        }
        else
        {
            $t->set_var( "expiry_value", $item_type->expiryTime() / 86400 );
            $t->set_var( "warning_value", $item_type->expiryTime() / 86400 );
        }
    }
}
