<?php
// 
// $Id: attributeedit.php,v 1.5 2001/07/19 12:19:20 jakobn Exp $
//
...
if( isset( $OK ) )
{
    ...
    for( $i = 0; $i < $count; $i++ )
    {
        $attribute = new eZRfpAttribute( $AttributeID[$i] );
        $attribute->setValue( $rfp, htmlspecialchars( $AttributeValue[$i] ) );
    }
...
if( $thisType->id() > 0 )
{
    ...
    if( $attributeCount > 0 )
    {
        foreach( $attributes as $attribute )
        {
            ...
            $t->set_var( "attribute_value", $attribute->value( $rfp ) );
