<?php
// 
// $Id: typeedit.php,v 1.6 2001/07/19 12:19:21 jakobn Exp $
//
...
if ( isset( $Cancel ) )
{
    eZHTTPTool::header( "Location: /rfp/type/list/" );
    exit();
}
...
if( isset( $Ok ) || isset( $NewAttribute ) )
{
    ...
    if ( count( $AttributeName ) > 0 )
    {
        ...
        {
            $att = new eZrfpAttribute( $AttributeID[$i] );
...
if( $Action == "up" )
{
    $attribute = new eZrfpAttribute( $AttributeID );
    $attribute->moveUp();
    include_once( "classes/ezhttptool.php" );
    eZHTTPTool::header( "Location: /rfp/type/edit/$TypeID/?$Action=update" );
    exit();
}
...
if ( isset ( $DeleteSelected ) )
{
    if ( count ( $DeleteAttributes ) > 0 )
    {
        ...
        {
            $attribute = new eZrfpAttribute( $attID );
            $attribute->delete();
        }
    }
    $Action = "edit";
...
$t = new eZTemplate( "ezrfp/admin/" . $ini->read_var( "eZrfpMain", "AdminTemplateDir" ),
                     "ezrfp/admin/intl/", $Language, "typeedit.php" );
