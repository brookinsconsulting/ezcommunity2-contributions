...
if ( !(function_exists('createRfpMenu') ) )
{ 
    function createRfpMenu( $menuCacheFile=false )
        {
            global $ini;
            global $Language;
            ...
            $t = new eZTemplate( "ezrfp/user/" . $ini->read_var( "eZRfpMain", "TemplateDir" ),
                                 "ezrfp/user/intl", $Language, "menubox.php" );
            ...
            $t->set_block( "menu_box_tpl", "rfp_category_tpl", "rfp_category" );
            $t->set_block( "menu_box_tpl", "submit_rfp_tpl", "submit_rfp" );

            $t->set_var( "submit_rfp", "" );
            ...
            $rfpCategory = new eZRfpCategory( $CategoryID );

            $rfpCategory_array = $rfpCategory->getByParent( $rfpCategory );

            $t->set_var( "top_title", $rfpCategory->name() );

            $i = 0;
            foreach( $rfpCategory_array as $categoryItem )
            {
                if( eZObjectPermission::hasPermission( $categoryItem->id(), "rfp_category", 'r' ) )
                {
                    $t->set_var( "rfpcategory_id", $categoryItem->id()  );
                    $t->set_var( "rfpcategory_title", $categoryItem->name() );

					if (
                        (( $url_array[2] == "archive" ) && ( $url_array[3] == $categoryItem->id() ))
                        || (( $url_array[2] == "rfpview" ) && ( $url_array[5] == $categoryItem->id() ))
                        )
					{
                    ...
                    $t->parse( "rfp_category", "rfp_category_tpl", true );
                    $i++;
                }
            }

            if ( $i == 0 )
                $t->set_var( "rfp_category", "" );
            ...
            {
                $t->parse( "submit_rfp", "submit_rfp_tpl", true );
            }
